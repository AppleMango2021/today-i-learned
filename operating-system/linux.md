...
---
 # Chapter 6. 프로세스와 시그널
컴퓨터에서의 프로세스는 실행중인 프로그램이라고 생각하면 된다. 운영체제의 가장 큰 역할 중에 하나가 
바로 이 프로세스를 잘 관리하는 것. 
프로그램은 하드디스크에만 존재하면 되고, 프로세스는 메모리 위에서 돌아간다. 
이때 프로그램은 하나지만, 하나의 프로그램은 여러 번 실행될 수 있고 그렇다는 건
여러 개의 프로세스가 동시에 존재할 수 있다. 
물론 프로세스가 해야할 일을 실제로 실행하는 것은 CPU! CPU는 여러 개의 프로세스를
돌아가면서 실행하게 되는데, 이를 위해서는 CPU는 프로그램 코드, 프로세스별 stack/heap 등을 
알고 접근할 수 있어야 한다. 

## 프로세스 스케쥴링 (Process Scheduling)
CPU의 관점에서 할 일들을 결정하는 것! 즉, 여러 개의 프로세스가 있는데 
이때 어느 걸 실행할지에 대한 순서를 결정하는 것이라고 보면 된다.
프로세스 스케쥴링이 잘 이루어져야 음악을 들으면서 크롬으로 검색도 하고, 넷플릭스도 
보는 등 동시에 여러 개의 작업을 할 수 있다. 
<br>
요즘의 컴퓨터에는 CPU가 여러 개가 있기 때문에(멀티코어), CPU 1번은 1번 프로세스를 작동하고 
있고 그와 동시에 CPU 2번은 4번 프로세스를 작동시키게 된다. 따라서 이론적으로 CPU가 늘어날 수록
컴퓨터의 성능은 향상된다. 개발을 잘 못한 어플리케이션의 경우, 코어의 갯수가 늘어나면 오히려 
성능이 떨어지는 경우가 발생할 수도 있다. 
 
## 프로세스 상태 (Process State)
![process-states.png](image/process-states.png)

- Waiting : CPU가 자기 자신을 처리해주기를 기다리는 상태. 이때 Waiting 상태에 있는 프로세스가 여러 개가 있을
  수 있는데, 어떠한 프로세스를 선택할 것인지도 알고리즘에 따라 다 다름. 
- Blocked : Waiting과 비슷하지만, 하드디스크 등과 같은 I/O 디스크와 통신을 하느라 대기하는 시간. 
  만약 이런 시간에도 해당 프로세스가 CPU를 점유하고 있으면 이는 정말 큰 시간 낭비!! 이때 다른 프로세스가 실행 
  될 수 있도록 Blocked 상태로 존재하는 것.
- Running : CPU에 의해서 실행되고 있는 상

## 프로세스의 계층구조
### PID (Process ID)
각 프로세스마다 부여받은 유니크한 ID값

### 부모 프로세스와 자식 프로세스
하나의 프로세스가 다른 프로세스를 생성할 수 있는데, 이러한 관계를 부모-자식이라고 한다. 
init프로세스를 제외한 **모든 프로세스는 부모 프로세스를 가지고 있다.**


### Init 프로세스
리눅스를 시작하면 커널이 메모리로 로드돼서 커널에 의해 부팅이 시작된다. 
이때 커널이 가장 먼저 하는 일은 하드웨어 자원들을 초기화한 이후에 프로세스 초기화 작업을 하는데,
그때 만들어내는 최초의 프로세스가 Init Process. 이 프로세스의 ID는 항상 1번이다.

### 프로세스의 종료 처리
근데 왜 프로세스들을 부모-자식과 같은 계층관계로 만들어 냈을까? 프로세스의 종료처리와 관련되어 있을 
것이라고 예상함.
프로세스가 종료될 때 그 프로세스에서 사용된 자원을 깔끔히 정리하는 책임을 부모 프로세스가 가지고 있는 것! 
만약 부모 프로세스에서 이를 제대로
수행하지 못했을 경우에는 고아 프로세스와 좀비 프로세스가 나타남.

### 고아 프로세스와 좀비 프로세스
부모가 먼저 종료되어 버린 프로세스를 **고아 프로세스**라고 일컫는다. 고아 프로세스의 문제는,
자신을 종료시켜 줄 부모 프로세스가 먼저 종료되어 버려 자신을 종료시켜줄 프로세스가 없는 것.
이런 문제가 생긴 경우, 고아 프로세스의 부모를 init 프로세스로 재설정한다. 그러면 리눅스가 종료될 때
고아 프로세스도 함께 깔끔히 종료되는 것.
<br>

부모 프로세스가 자식 프로세스를 종료시킨 다고 했는데, 이를 수행하는 방법에는 크게 2가지가 있다.
첫 번째로는, 자식 프로세스가 완전히 종료될 때까지 부모 프로세스가 대기하고 있는 것. 두 번째로는
자식 프로세스가 종료되면 운영체제가 부모 프로세스에게 시그널을 날려주고, 그때 부모 프로세스는 자식 프로세스에
대한 종료 처리를 하는 것. 두 번째가 조금 더 일반적임. 

좀비 프로세스란, 자식 프로세스가 종료가 됐는데 부모 프로세스는 위의 두 가지를 모두 안 하는 것. 
그러면 자식 프로세스는 반쯤 종료된 상태로 대기하고 있기 때문에 이를 좀비 프로세스라고 한다. 어쨌든
완전히 사라진 것은 아니기 때문에 메모리든지, PID라든지 시스템의 자원들을 차지하고 있는 것. 

## 프로세스 종료 상태 (Exit Status)
- 자식 프로세스가 종료되면서 부모 프로세스에게 넘겨주는 정보
    - 정상 종료 여부
    - 시그널을 받아 종료되었는지 여부 및 시그널의 종류
    - 코어 덤프(core dump)를 생성했는지 여부
- 자식 프로세스의 성공/실패 여부에 따라서 부모 프로세스가 다르게 작동하는 로직이 있을 수 있기 때문에 
  자식 프로세스의 작동을 잘 살펴봐야 한다.
- 종료 상태값은 integer로 받는데, `0`은 `성공`을 의미하며 `0이 아닌 다른 값`은 `실패`로 인지한다

## 표준 스트림 (Standard Stream)
- 모든 프로세스가 가지고 있는 입출력 창구 
    - 프로세스는 키보드, 모니터 등 다른 디바이스와 통신을 해야 하는 일이 생기는데 그때 활용할 수 있는 소통 창구
- 모든 프로세스가 생성이 되면 각각의 프로세스는 아래 3개의 스트림을 가지게 된다. 이때 각 스트림을 구분할 수 있는
  Integer타입의 Identifier가 있는데, 아래와 같이 각각 0/1/2번을 가진다. (스트림도 일종의 파일로 인식한다)   
    - 표준 입력 (Standard Input, #0) 
    - 표준 출력 (Standard Output, #1)
    - 표준 에러 (Standard Error, #2)

## 파일 디스크립터 (File Descriptor)
- 파일을 대변하는 객체로 파일에 대한 작업(읽기/쓰기)을 하기 위해서는 파일 디스크립터를 열어야 함
- 각 프로세스는 본인이 열은 파일 디스크립터 목록을 관리한다
    - 이때, 0/1/2번은 위에서 말했듯이 스트림을 가리키게 된다 

## 포어/백그라운드(Foreground/Background) 프로세스
- 포어그라운드 프로세스
...
- `보이지 않는 곳에서 어떤 일을 하는 유령`이라는 단어에서 영감을 받아 이름지은 프로세스
- 백그라운드 프로세스로 동작하기 위해 만들어진 프로세스
- 관용적으로 데몬 프로세스는 'd'로 끝난다 (ex) systemd  
- 표준 스트림을 가지고 시작하지만 바로 모두 닫아 버림(-> 쉘과의 입출력 교환이 불가능함)
- 자기를 띄워준 부모 프로세스는 init 프로세스로 변경함
- (ex) 시스템에서 사용자와의 인터랙션 없이 모니터링을 하는 프로세스 등

## 시그널(Signal)
- **비동기 이벤트**를 처리하기 위한 프로세스간 통신(Inter-Process Communication)
    - 프로세스 A가 프로세스 B에게 시그널을 보내면, 프로세스 B로부터 응답을 받기를 기다리지 않음
    - 소켓 등과 같이 데이터를 주고 받으려는 용도보다는 말 그대로 일방적으로 간단한 정보를 알려주려고 할 때 많이 사용
    - 통상적으로 시그널 번호를 보내주며, 보낼 수 있는 번호는 정해져 있다.
      
        | 시그널 이름 | 의미  | 기본 처리 |
        |----------|------|--------|
        | SIGABRT | abort()에서 보냄. 의도적인 중단을 의미 | 코어 덤프 |
        | SIGCHLD | 자식 프로세스가 종료됨 | 무시 |
        | SIGINT | 사용자가 인터럽트(ex. `Ctrl+C`)를 생성 | 종료 | 
        | SIGKILL | 프로세스 종료 명령 | 종료 |
        | SIGTERM | 프로세스 종료 명령 (조건에 따른 처리 가능) | 종료 |
    
        * 기본 처리 : 프로세스에서 별도의 시그널 핸들링을 하지 않았을 때의 처리 행위

## 프로세스 관련 실습
### ps (Process)
현재 프로세스의 목록을 보여주는 명령어
```shell
$ ps #아무런 옵션없이 ps만 넣으면 현재 터미널에서 실행된 프로세스만 보여준다
> PID TTY           TIME CMD
>10002 ttys008    0:00.10 -zsh
## 위에서 TIME은 해당 프로세스가 생성된 시간 같은 게 아니고, 해당 프로세스가 CPU를 점유하던 시간!

$ ps -f #아무 옵션 없는 것보다는 많은 정보를 준다
>   UID   PID  PPID   C STIME   TTY           TIME CMD
>   501 10002 10001   0  4:19PM ttys008    0:00.22 -zsh
# PPID : Parent의 PID
# C : CPU Usage(%)
# STIME : 프로세스가 실행되기 시작한 시간
# TIME : CPU 점유 시간

$ ps -ef #가장 많이 사용하는 명령어. 현재 터미널에서 시작된 프로세스 말고, 모든 프로세스를 보여줌 
```

---

# Chapter 7. 리디렉션과 파이프라인
리눅스 보다는 Bash shell에서 지원하는 기능이지만, 빈번하게 사용되기 때문에 알아두면 좋음
- 리디렉션 : 화면 상에 출력될 내용을 File, File Descriptor에 저장하거나 Pipeline 등을 통해 다른 File Descriptor에
연결해주는 기능

## 출력 리디렉션 (`>`)
- 화면에 출력될 내용을 다른 곳으로 보내는 것 
- 문법 : `[n]>[|] 파일명`
    - `n` : 어떤 스트림을 파일에 저장할 것인지 표기하는 것. 생략하면 표준 출력(1)을 파일로 리디렉션한다. 
        - 1 : 표준 출력 스트림
        - 2 : 표준 에러 스트림
- 대상 파일이 존재하는 경우에는 파일을 덮어쓰게 된다
    - noclobber 옵션을 설정하는 경우, 덮어쓰기 시도할 때 에러가 발생한다
    - noclobber 옵션을 설정하더라도 `>|` 사용 시, 무조건 덮어쓰게 된다 
```shell
# ls의 표준 출력을 test.txt 파일에 옮겨놓는 것 
$ ls > test.txt

# ls의 에러를 test.txt 파일에 옮겨놓는 것
$ ls /sdfkew 2> test.txt
```

## 추가 모드 출력 리디렉션(`>>`)
- 출력 리디렉션(`>`)은 기본적으로 덮어쓰는데, 추가 모드 출력 리디렉션의 경우에는 append를 하게 되는 것
- 문법 : `[n]>> 파일명`
    - `n` : 어떤 스트림을 파일에 저장할 것인지 표기하는 것. 생략하면 표준 출력(1)을 파일로 리디렉션한다.
        - 1 : 표준 출력 스트림
        - 2 : 표준 에러 스트림
...
## 파일 디스크립터로 리디렉션 (`>&`)
- 파일로 리디렉션하는 것과 동일하지만, 대상 파일 대신에 대상 파일의 디스크립터를 지정한다.
- 표준 출력과 표준 에러를 한꺼번에 출력하고 싶을 때 자주 사용  
- 문법 : `[n]>& 파일디스크립터명`
```shell
# ls의 표준 출력을 result.txt 파일에 옮겨놓되, 만약 에러가 출력되면 그 에러도 표준 출력으로 리디렉션해! 
# 조금 어색한 느낌이 있지만, 다른 사람이 작성한 스크립트를 보면 나오는 경우가 종종 있음.
# 내가 작성할 때에는 아래 동시 리디렉션을 사용하는 것을 추천함
$ ls > result.txt 2>&1
```

## 표준 출력 및 표준 에러를 동시 리디렉션 (`&>`)
- 파일 디스크립터 리디렉션 문법이 귀찮고 어렵기 때문에 이를 사용하면 좀 더 편하게 할 수 있다
- 문법 : `&> 파일명`

## 입력 리디렉션 (`<`)
- 파일의 내용이 지정된 스트림으로 리디렉션 
- n이 생략되면 표준 입력(0)을 의미   
- 문법 : `[n]<파일명`
```shell
# wc 명령어는 입력 스트림의 라인 수, 단어 수 등을 카운트 하는 명령어
$ wc < result.txt
```

## Here documents (`<<`)
- 프로그램의 표준 입력으로 multi-line string을 전달 
- 표준 입력으로 전달되어야 할 내용이 파일에 저장되어 있지 않고, 내가 직접 여러 라인을 입력해야 할 때 많이 사용됨
- 문법 : `<<DELIM`
    - 여기에서 DELIM은 다른 단어로 변경 가능하다 (AAA, END 등 모든 단어 다 가능)
```shell
$ cat > text.txt << DELIM
(내용입력)
(내용입력)
...
## Here strings (`<<<`)
- Here document의 한 줄 버전인데 자주 사용되지는 않음 

## 파이프라인 (`|`)
- 프로그램간 상호작용을 할 때 사용
- A 프로세스의 스트림과 B 프로세스의 스트림을 연결해줄 때 사용
- 문법 
    - `command1 | command2` : command1의 표준 출력이 command2의 표준 입력으로 연결됨
    - `command1 |& command2` : command1의 표준 출력과 표준에러가 command2의 표준 입력으로 연결됨
- 각 command는 sub-shell에서 실행되며, 전체 파이프라인의 exit status는 마지막 실행 커맨드의 exit status임
- pipefail 옵션이 설정되어 있다면, 실패한 커맨드가 존재하는 경우 실패로 처리됨
```shell
# cat 명령어로 인해 result.txt를 출력하게 되고, 
# wc는 해당 명령어만 입력했을 때 표준 입력을 기다리고 있기 때문에
# 파이프라인으로 연결하게 되면 result.txt의 word count 결과를 보여준다 
$ cat result.txt | wc

# 결과물 자체는 아래와 동일하다
$ wc < result.txt

# 아래는 /etc/passwd에서 'root' 단어를 찾는 여러 방법
